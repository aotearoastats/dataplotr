plot_barplot <-
  function(data_frame,
    ...
    if (!(colour_title == FALSE)) {
      p <-
        p + ggplot2::guides(colour = ggplot2::guide_legend(title = colour_title, reverse = legend_rev))
    }
    
    if (!(fill_title == FALSE)) {
      p <-
        p + ggplot2::guides(fill = ggplot2::guide_legend(title = fill_title, reverse = legend_rev))
    }
    ...
    if (!is.null(main_title)) {
      cow <- cowplot::ggdraw()
      cow <-
        cow + cowplot::draw_label(main_title, size = ggplot2::theme_get()[["main.title.size"]])
      p <-
        cowplot::plot_grid(cow, p, ncol = 1, rel_heights = c(0.1, 1))
    }
